...
class ObjectClassificationNode(Node):
    ...
    def get_min_distance(self, depth_image):
        # 打印深度图像的基本信息
        self.get_logger().info(f"深度图像形状: {depth_image.shape}")
        self.get_logger().info(f"深度图像数据类型: {depth_image.dtype}")
        self.get_logger().info(f"深度值范围: 最小={np.min(depth_image[depth_image > 0])}mm, 最大={np.max(depth_image)}mm")
        
        # 打印ROI区域的深度信息
        roi_depth = depth_image[self.roi[0]:self.roi[1], self.roi[2]:self.roi[3]]
        self.get_logger().info(f"ROI区域形状: {roi_depth.shape}")
        valid_roi = roi_depth[roi_depth > 0]  # 排除0值（无效深度）
        if len(valid_roi) > 0:
            self.get_logger().info(f"ROI区域深度值范围: 最小={np.min(valid_roi)}mm, 最大={np.max(valid_roi)}mm")
            self.get_logger().info(f"ROI区域平均深度: {np.mean(valid_roi):.2f}mm")
        
        # 打印几个示例点的深度值
        center_y = (self.roi[0] + self.roi[1]) // 2
        center_x = (self.roi[2] + self.roi[3]) // 2
        self.get_logger().info(f"ROI中心点({center_x}, {center_y})的深度: {depth_image[center_y, center_x]}mm")
        
        # 打印ROI四角的深度值
        self.get_logger().info(f"ROI左上角({self.roi[2]}, {self.roi[0]})的深度: {depth_image[self.roi[0], self.roi[2]]}mm")
        self.get_logger().info(f"ROI右上角({self.roi[3]}, {self.roi[0]})的深度: {depth_image[self.roi[0], self.roi[3]-1]}mm")
        self.get_logger().info(f"ROI左下角({self.roi[2]}, {self.roi[1]})的深度: {depth_image[self.roi[1]-1, self.roi[2]]}mm")
        self.get_logger().info(f"ROI右下角({self.roi[3]}, {self.roi[1]})的深度: {depth_image[self.roi[1]-1, self.roi[3]-1]}mm")
    ...
    def get_contours(self, depth_image, min_dist):
        try:
            self.get_logger().info(f"开始获取轮廓，输入深度图像形状: {depth_image.shape}, 最小距离: {min_dist}mm")
            
            if self.plane_distance is None:
                self.get_logger().error("self.plane_distance为None，无法处理")
                self.plane_distance = 1000
            self.get_logger().info(f"使用平面距离: {self.plane_distance}mm")
            
            # 过滤掉太远的背景
            depth_image_filtered1 = np.where(depth_image > self.plane_distance - 15, 0, depth_image)
            self.get_logger().info(f"过滤掉深度值 > {self.plane_distance - 15}mm 的像素")
            
            # 过滤掉不在目标物体附近的像素
            depth_image_filtered2 = np.where(depth_image_filtered1 > min_dist + 40, 0, depth_image_filtered1)
            self.get_logger().info(f"过滤掉深度值 > {min_dist + 23}mm 的像素")
            
            # 统计有效像素数量
            valid_pixels = np.count_nonzero(depth_image_filtered2)
            self.get_logger().info(f"过滤后剩余有效像素数量: {valid_pixels}")
            
            # 归一化深度图像
            max_depth = self.plane_distance - 10
            sim_depth_image_sort = np.clip(depth_image_filtered2, 0, max_depth).astype(np.float64) / max_depth * 255
            self.get_logger().info(f"深度图像归一化，最大深度值: {max_depth}mm")
            
            # 转换为8位灰度图
            depth_gray = sim_depth_image_sort.astype(np.uint8)
            self.get_logger().info(f"转换为8位灰度图，灰度值范围: [{np.min(depth_gray)}, {np.max(depth_gray)}]")
            
            # 二值化处理
            _, depth_bit = cv2.threshold(depth_gray, 1, 255, cv2.THRESH_BINARY)
            white_pixels = np.count_nonzero(depth_bit)
            self.get_logger().info(f"二值化处理，阈值: 1，白色像素数量: {white_pixels}")
            ...
            for i, contour in enumerate(contours):
                ...
                # 只保留面积在2000-2400范围内的轮廓
                if 2000 <= area <= 3000:
                    perimeter = cv2.arcLength(contour, True)
                    (cx, cy), radius = cv2.minEnclosingCircle(contour)
                    center, (width, height), angle = cv2.minAreaRect(contour)
                    self.get_logger().info(f"符合条件的轮廓: 面积={area:.2f}, 周长={perimeter:.2f}, "
                                          f"中心=({center[0]:.2f}, {center[1]:.2f}), "
                                          f"宽高=({width:.2f}, {height:.2f}), 角度={angle:.2f}")
                    filtered_contours.append(contour)
                else:
                    self.get_logger().info(f"跳过轮廓: 面积={area:.2f}，不在目标范围内")
    ...
    def shape_recognition(self, rgb_image, depth_image, depth_color_map, intrinsic_matrix, min_dist):
        ...
        # 检查最小距离是否小于205mm
        if min_dist <= 205:
            self.get_logger().info(f"最小距离 {min_dist}mm 小于阈值205mm，进行物体识别")
            sphere_index = 0
            cuboid_index = 0
            cylinder_index = 0
            ...
            self.get_logger().info(f"最小距离 {min_dist}mm 大于阈值205mm，跳过物体识别")
            # 当没有符合条件的物体时，清除目标形状
            self.target_shapes = ''
