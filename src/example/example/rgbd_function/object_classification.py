...
class ShapeRecognitionNode(Node):
    ...
    def go_home(self, interrupt=True):
        ...
        set_servo_position(self.joints_pub, 1, ((1, joint_angle[0]), (10, 700)))
        time.sleep(1)
    ...
    def enter_srv_callback(self, request, response):
        ...
        set_servo_position(self.joints_pub, 1, ((1, joint_angle[0]), (2, joint_angle[1]), (3, joint_angle[2]), (4, joint_angle[3]), (5, 500), (10, 700)))
        msg = set_joint_value_target(joint_angle)
        endpoint = self.send_request(self.set_joint_value_target_client, msg)
        pose_t, pose_r = endpoint.pose.position, endpoint.pose.orientation
        self.endpoint = common.xyz_quat_to_mat([pose_t.x, pose_t.y, pose_t.z], [pose_r.w, pose_r.x, pose_r.y, pose_r.z])
    ...
    def transport_thread(self):
        while self.running:
            if self.start_transport:
                ...
                if "sphere" in shape:
                    finish = pick_and_place.pick(position, 85, yaw, 430, 0.05, self.joints_pub, self.kinematics_client)
                else:
                    finish = pick_and_place.pick(position, 85, yaw, 430, 0.02, self.joints_pub, self.kinematics_client)
                if finish:
