...
class RemoveTooHighObjectNode(Node):
    ...
    def __init__(self, name):
        ...
        self.start_process = False # 添加一个标志位来控制是否开始处理
    ...
    def init_process(self):
        ...
        #set_servo_position(self.joints_pub, 1, ((6, 500), (5, 540), (4, 780), (3, 50), (2, 500), (1, 200)))
    ...
    def pick(self, position, angle):
        ...
        self.get_logger().info(f"Complete res object: {res}")  
        self.get_logger().info(f"res.rpy: {res.rpy}") 
        self.get_logger().info(f"Length of res.rpy: {len(res.rpy)}")  
        if res.pulse:
        ...
        msg = set_pose_target(position, yaw, [-180.0, 180.0], 1.0)
        res = self.send_request(self.set_pose_target_client, msg)
        self.get_logger().info(f"Complete res object after third call: {res}")
        self.get_logger().info(f"res.rpy: {res.rpy}")
        if res.pulse:
            servo_data = res.pulse
            set_servo_position(self.joints_pub, 1.5, ((6, servo_data[0]),(5, servo_data[1]), (4, servo_data[2]),(3, servo_data[3])))
        ...
        while self.running:
            ...
            try:
                ...
                for obj in contours:
                    ...
                    # 将位置信息格式化为字符串，保留两位小数
                    position_text = f"x:{position[0]:.2f}, y:{position[1]:.2f}, z:{position[2]:.2f}" 
                ...
                if largest is not None:
                    # 在图像上显示坐标信息
                    cv2.putText(bgr_image, position_text, (10, ih - 50), cv2.FONT_HERSHEY_PLAIN, 2.0, (32, 32, 32), 6, cv2.LINE_AA)
                    cv2.putText(bgr_image, position_text, (10, ih - 50), cv2.FONT_HERSHEY_PLAIN, 2.0, (240, 240, 240), 2, cv2.LINE_AA)
                    obj, pose_t = largest
                    ...
                    if dist < 0.002 and 0.01 < pose_t[2] and self.start_process: # 只有按下's' 键后才开始运行
                ...
                key = cv2.waitKey(1)
                if key == ord('s'): # 按下 's' 键开始运行
                    self.start_process = True
                    self.get_logger().info("开始处理")
                if key != -1:
