...
class AutomaticPickNode(Node):
    ...
    def __init__(self, name):
        ...
        self.linear_pid = PID(P=0.0008, I=0, D=0)
        ...
        set_servo_position(self.joints_pub, 2, ((1, 500), (2, 700), (3, 15), (4, 175), (5, 500), (10, 300)))
        time.sleep(2)
        # msg = Trigger.Request()
        # self.start_pick_callback(msg, Trigger.Response())
        self.get_logger().info('\033[1;32m self.debug%s \033[0m' % str(self.debug))
        if self.debug == 'pick':
    ...
    def pick_handle(self, image):
        ...
        if not self.pick or self.debug == 'pick':
            ...
            self.get_logger().info('x_y: ' + str([object_center_x, object_center_y]))
            if self.debug == 'pick':
                self.detect_count += 1
    ...
    def main(self):
        while self.running:
            ...
            if self.box:
                ...
                # else:
                self.get_logger().info('\033[1;32m%s\033[0m' % 'large get box: ' + str(self.box))
                if self.start_pick:
                    point = Point()
                    ...
                    # cv2.waitKey(5000)
                    self.get_logger().info('point.x, point.y: ' + str([point.x, point.y]))
